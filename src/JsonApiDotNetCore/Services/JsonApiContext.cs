...
namespace JsonApiDotNetCore.Services
{
  public class JsonApiContext : IJsonApiContext
  {
    ...
    public ILinkBuilder LinkBuilder { get; set; }
    ...
    [Obsolete("Use ApplyContextDI which uses ILinkBuilder instead")]
    ...
    public IJsonApiContext ApplyContextDI<T>(object controller)
    {
      if (controller == null)
        throw new JsonApiException(500, $"Cannot ApplyContext from null controller for type {typeof(T)}");
      ...
      BasePath = LinkBuilder.GetBasePath(context, Options, _controllerContext.RequestEntity.EntityName);
      PageManager = GetPageManager();
      IsRelationshipPath = PathIsRelationship(context.Request.Path.Value);
